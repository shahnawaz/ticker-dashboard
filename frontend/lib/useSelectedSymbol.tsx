...
export function SelectedSymbolProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('symbol'); if (saved) setSelected(saved)
  }, [])
  useEffect(() => { if (selected) localStorage.setItem('symbol', selected) }, [selected])
  return <C.Provider value={{ selected, setSelected }}>{children}</C.Provider>
}

export function useSelectedSymbol(){ return useContext(C) }
